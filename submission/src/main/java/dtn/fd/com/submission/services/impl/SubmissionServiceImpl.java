...
public class SubmissionServiceImpl implements ISubmissionService {
    ...
    public Submission getSubmissionById(String serialNumber) {
        return submissionList.stream().filter(submission -> submission.getSerialNumber().equals(serialNumber.trim())).findFirst().orElseThrow(()->new RuntimeException("ivalid serial number"));
    }

    @Override
    public List<Submission> searchSubmissionByLead(String lead) {
        Pattern pattern = Pattern.compile(lead, Pattern.CASE_INSENSITIVE);
        return submissionList.stream().filter(submission -> pattern.matcher(submission.getLead()).find()).collect(Collectors.toList());
    }
